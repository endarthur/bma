...
function renderCrosstabSankey(data) {
  ...
  for (var i2 = 0; i2 < nA; i2++) {
    for (var j2 = 0; j2 < nB; j2++) {
      ...
      svg += '<path class="crosstab-flow" d="' + path + '" fill="hsl(' + crosstabHue(i2) + ',55%,55%)" fill-opacity="0.34">' +
        '<title>' + esc(aL[i2]) + ' → ' + esc(bL[j2]) + ': ' + v.toLocaleString() + '</title></path>';
    }
  }
  // nodes + labels
  for (var i3 = 0; i3 < nA; i3++) {
    var nh = Math.max(rowTot[i3] * scale, 1);
    svg += '<rect class="crosstab-node" x="' + lx + '" y="' + aY[i3] + '" width="' + nodeW + '" height="' + nh + '" fill="hsl(' + crosstabHue(i3) + ',55%,50%)"><title>' + esc(aL[i3]) + ': ' + rowTot[i3].toLocaleString() + '</title></rect>';
  ...
  for (var j3 = 0; j3 < nB; j3++) {
    ...
    svg += '<rect class="crosstab-node" x="' + rx + '" y="' + bY[j3] + '" width="' + nodeW + '" height="' + nh2 + '" fill="hsl(' + crosstabHue(j3) + ',30%,50%)"><title>' + esc(bL[j3]) + ': ' + colTot[j3].toLocaleString() + '</title></rect>';
