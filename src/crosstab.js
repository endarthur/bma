...
// distribution shown four ways (cross TABLE w/ margins · confusion-matrix heatmap
// + Cohen's κ · Sankey flows · grouped/stacked bars), optionally tonnage-weighted.
// Runs its own worker pass (mode 'crosstab') over the target dataset, mirroring
// the GT/Swath "own worker" panels. The first day-1 friend-feedback feature on
// v1.3. Design: roadmap A19.
//
// CLONEABLE (A19-clone): like Swath/GT, each clone is a FULLY independent
// cross-tab (own target, columns, weight, view, worker, results). The panel is
// template-structured (DOM in template.html) like Categories/Statistics, so DOM
// resolves through a per-instance ROOT (data-xt markers) and state through
// crosstabStateForRoot(root): the SINGLETON proxies to the module globals (so
// project.js serialize/restore stays bit-identical), a CLONE gets a plain object.
...
// ── DOM resolution through a per-instance root (singleton = #panelCrosstab) ──
function crosstabPanelRoot() { return document.getElementById('panelCrosstab'); }
function crosstabSidebarEl(root) { var r = root || crosstabPanelRoot(); return r ? r.querySelector('[data-xt="sidebar"]') : null; }
function crosstabContentEl(root) { var r = root || crosstabPanelRoot(); return r ? r.querySelector('[data-xt="content"]') : null; }

// ── per-instance state ──
var crosstabInstances = {};
function crosstabNewInstState() {
  return { targetDsId: 'model', colA: null, colB: null, weightCol: null, view: 'table', barMode: 'stacked', cellMode: 'count', worker: null, lastData: null };
}
// The singleton's state is an accessor proxy over the module globals, so external
// readers (project.js serialize/restore) stay bit-identical.
var _crosstabSingleton = {
  get targetDsId() { return crosstabTargetDsId; }, set targetDsId(v) { crosstabTargetDsId = v; },
  get colA() { return crosstabColA; }, set colA(v) { crosstabColA = v; },
  get colB() { return crosstabColB; }, set colB(v) { crosstabColB = v; },
  get weightCol() { return crosstabWeightCol; }, set weightCol(v) { crosstabWeightCol = v; },
  get view() { return crosstabView; }, set view(v) { crosstabView = v; },
  get barMode() { return crosstabBarMode; }, set barMode(v) { crosstabBarMode = v; },
  get cellMode() { return crosstabCellMode; }, set cellMode(v) { crosstabCellMode = v; },
  get worker() { return crosstabWorker; }, set worker(v) { crosstabWorker = v; },
  get lastData() { return lastCrosstabData; }, set lastData(v) { lastCrosstabData = v; }
};
function crosstabStateForRoot(root) {
  var id = root && root.getAttribute ? root.getAttribute('data-xt-inst') : null;
  if (id && crosstabInstances[id]) return crosstabInstances[id];
  return _crosstabSingleton;
}
...
function crosstabTargetDs(root) {
  return surfaceTarget('categorical', crosstabStateForRoot(root).targetDsId, { keepUnanalyzed: true });
}
...
function crosstabCtx(root) {
  var ds = crosstabTargetDs(root);
  if (!ds) return null;
...
function crosstabSetStatus(msg, isErr, root) {
  var sb = crosstabSidebarEl(root);
  var el = sb && sb.querySelector('[data-xt="status"]');
  if (el) { el.textContent = msg || ''; el.style.color = isErr ? 'var(--red)' : ''; }
}

function renderCrosstab(root) {
  var sb = crosstabSidebarEl(root);
  if (!sb) return;
  var S = crosstabStateForRoot(root);
  var ctx = crosstabCtx(root);
  ...
  if (!ctx || !ctx.file || cols.length < 2) {
    ...
    var content = crosstabContentEl(root);
    if (content && !S.lastData) content.innerHTML = '<div class="crosstab-hint">Pick two categorical columns and click Generate to cross-tabulate them.</div>';
    return;
  }
  // default the two columns to the first two distinct categoricals
  if (S.colA == null || !cols.some(function (c) { return c.idx === S.colA; })) S.colA = cols[0].idx;
  if (S.colB == null || !cols.some(function (c) { return c.idx === S.colB; })) S.colB = (cols[1] || cols[0]).idx;
  ...
  if (S.weightCol != null && !numCols.some(function (c) { return c.idx === S.weightCol; })) S.weightCol = null;
  var weightOptions = '<option value="">— none (count)</option>' + numCols.map(function (c) {
    return '<option value="' + c.idx + '"' + (c.idx === S.weightCol ? ' selected' : '') + '>' + esc(c.name) + '</option>';
  }).join('');
  ...
  sb.innerHTML =
    ...
    '<div class="crosstab-section"><div class="crosstab-sidebar-title">Rows (A)</div>' +
      '<select class="stats-select" data-xt="colA">' + colOptions(S.colA) + '</select></div>' +
    '<div class="crosstab-section"><div class="crosstab-sidebar-title">Columns (B)</div>' +
      '<select class="stats-select" data-xt="colB">' + colOptions(S.colB) + '</select></div>' +
    ...
    '<div class="crosstab-section"><div class="crosstab-sidebar-title">View</div>' +
      '<select class="stats-select" data-xt="view">' +
        ['table:Cross-table', 'heatmap:Confusion matrix', 'sankey:Sankey', 'bars:Bars'].map(function (o) {
          var p = o.split(':'); return '<option value="' + p[0] + '"' + (p[0] === S.view ? ' selected' : '') + '>' + p[1] + '</option>';
        }).join('') +
      '</select></div>' +
    '<div class="crosstab-section"><div class="crosstab-sidebar-title">Show</div>' +
      '<select class="stats-select" data-xt="cellMode">' +
        ['count:Counts', 'rowpct:Row %', 'colpct:Column %', 'totalpct:Total %'].map(function (o) {
          var p = o.split(':'); return '<option value="' + p[0] + '"' + (p[0] === S.cellMode ? ' selected' : '') + '>' + p[1] + '</option>';
        }).join('') +
      '</select></div>' +
...
function wireCrosstab(root) {
  var sb = crosstabSidebarEl(root);
  if (!sb) return;
  var S = crosstabStateForRoot(root);
  var dsSel = sb.querySelector('[data-xt="datasetSel"]');
  if (dsSel) dsSel.onchange = function () {
    S.targetDsId = dsSel.value; S.colA = null; S.colB = null; S.lastData = null;
    renderCrosstab(root);
    var content = crosstabContentEl(root);
  ...
  var a = sb.querySelector('[data-xt="colA"]'); if (a) a.onchange = function () { S.colA = parseInt(a.value); autoSaveProject && autoSaveProject(); };
  var b = sb.querySelector('[data-xt="colB"]'); if (b) b.onchange = function () { S.colB = parseInt(b.value); autoSaveProject && autoSaveProject(); };
  var wt = sb.querySelector('[data-xt="weight"]'); if (wt) wt.onchange = function () { S.weightCol = wt.value === '' ? null : parseInt(wt.value); autoSaveProject && autoSaveProject(); };
  var vw = sb.querySelector('[data-xt="view"]'); if (vw) vw.onchange = function () {
    S.view = vw.value;
    if (S.lastData) renderCrosstabResult(S.lastData, root);   // re-render from cache (no re-run)
    autoSaveProject && autoSaveProject();
  };
  var cm = sb.querySelector('[data-xt="cellMode"]'); if (cm) cm.onchange = function () {
    S.cellMode = cm.value;
    if (S.lastData) renderCrosstabResult(S.lastData, root);
    autoSaveProject && autoSaveProject();
  };
  var gen = sb.querySelector('[data-xt="generate"]'); if (gen) gen.onclick = function () { runCrosstab(root); };
}

function runCrosstab(root) {
  var S = crosstabStateForRoot(root);
  var ctx = crosstabCtx(root);
  if (!ctx || !ctx.file) { crosstabSetStatus('Analyze a dataset first', true, root); return; }
  if (S.colA == null || S.colB == null) { crosstabSetStatus('Pick two columns', true, root); return; }
  ...
  crosstabSetStatus('', false, root);

  S.worker = new Worker(workerUrl);
  S.worker.postMessage({
    ...
    colA: S.colA, colB: S.colB,
    weightCol: S.weightCol,   // A19-5: tonnage/volume weighting (null = count)
  ...
  S.worker.onmessage = function (e) {
    ...
    if (m.type === 'crosstab-progress') {
      ...
      try { S.worker.terminate(); } catch (e2) {}
      S.worker = null;
      if (prog) prog.classList.remove('active');
      if (gen) gen.disabled = false;
      S.lastData = m;
      renderCrosstabResult(m, root);
      crosstabBadge(root, m);
      autoSaveProject && autoSaveProject();
      ...
      if (prog) prog.classList.remove('active');
      if (gen) gen.disabled = false;
      crosstabSetStatus(m.message || 'Error', true, root);
    }
  };
  S.worker.onerror = function (e) {
    if (prog) prog.classList.remove('active');
    if (gen) gen.disabled = false;
    crosstabSetStatus('Worker error: ' + (e.message || 'unknown'), true, root);
  };
}

// Result-count badge — the singleton writes the legacy/rails tab; a clone updates
// its own rails tab title.
function crosstabBadge(root, m) {
  var instId = root && root.getAttribute ? root.getAttribute('data-xt-inst') : null;
  var n = m.kept ? m.kept.toLocaleString() : null;
  if (instId && wsRails && typeof wsRails.updateTab === 'function') {
    wsRails.updateTab(instId, 'Cross-tab' + (n ? ': ' + n : ''));
  } else if (typeof wsTabBadge === 'function') {
    wsTabBadge('crosstab', 'Cross-tab', n);
  }
}
...
function renderCrosstabResult(data, root) {
  var content = crosstabContentEl(root);
  if (!content) return;
  ...
  var view = crosstabStateForRoot(root).view;
  if (view === 'heatmap') return renderCrosstabHeatmap(data, root);
  if (view === 'sankey') return renderCrosstabSankey(data, root);
  if (view === 'bars') return renderCrosstabBars(data, root);
  return renderCrosstabTable(data, root);
}
...
function crosstabFmtCell(v, rowTot, colTot, grand, cellMode) {
  if (cellMode === 'count') return v ? crosstabFmtNum(v) : '·';
  var den = cellMode === 'rowpct' ? rowTot : cellMode === 'colpct' ? colTot : grand;
  if (!den) return '·';
  var pct = (v / den) * 100;
...
function renderCrosstabTable(data, root) {
  var content = crosstabContentEl(root);
  var cellMode = crosstabStateForRoot(root).cellMode;
  var aL = data.aLabels, bL = data.bLabels, M = crosstabMatrix(data);
  ...
  for (var i2 = 0; i2 < aL.length; i2++) {
    ...
    for (var j3 = 0; j3 < bL.length; j3++) {
      ...
      html += '<td class="crosstab-cell' + diag + '" style="background:rgba(127,127,127,' + alpha + ')">' + crosstabFmtCell(v, rowTot[i2], colTot[j3], grand, cellMode) + '</td>';
    }
...
function renderCrosstabHeatmap(data, root) {
  var content = crosstabContentEl(root);
  var cellMode = crosstabStateForRoot(root).cellMode;
  var aL = data.aLabels, bL = data.bLabels, M = crosstabMatrix(data);
  var m = crosstabMargins(data), rowTot = m.rowTot, colTot = m.colTot, grand = m.grand;
  ...
  for (var i2 = 0; i2 < aL.length; i2++) {
    ...
    for (var j3 = 0; j3 < bL.length; j3++) {
      ...
      html += '<td class="crosstab-hcell' + (diag ? ' crosstab-diag' : '') + '" style="background:' + bg + '">' +
        crosstabFmtCell(v, rowTot[i2], colTot[j3], grand, cellMode) + '</td>';
    }
    html += '</tr>';
  }
  html += '</tbody></table></div>';
...
function renderCrosstabSankey(data, root) {
  var content = crosstabContentEl(root);
  var aL = data.aLabels, bL = data.bLabels, M = crosstabMatrix(data);
  var m = crosstabMargins(data), rowTot = m.rowTot, colTot = m.colTot, grand = m.grand;
...
function renderCrosstabBars(data, root) {
  var content = crosstabContentEl(root);
  var S = crosstabStateForRoot(root);
  var aL = data.aLabels, bL = data.bLabels, M = crosstabMatrix(data);
  ...
  var nA = aL.length, nB = bL.length;
  var pct = (S.cellMode !== 'count');
  var grouped = (S.barMode === 'grouped');
  ...
  content.querySelectorAll('.crosstab-bar-toggle button').forEach(function (btn) {
    btn.onclick = function () {
      S.barMode = btn.getAttribute('data-bar');
      if (S.lastData) renderCrosstabResult(S.lastData, root);
      autoSaveProject && autoSaveProject();
    };
  });
