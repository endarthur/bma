...
// So N Swath panels can coexist, every swath DOM lookup resolves through an
// optional per-instance ROOT instead of the document. The SINGLETON (root
// null/undefined) falls through to getElementById — bit-identical until clones
// exist. A clone (s-4) carries data-sw-inst and its generated controls are
// tagged data-sw="<id>" (the singleton keeps raw ids because project.js reads
...
// s-4a: renderSwathConfig emits its controls with data-sw="<id>" (duplicate-safe
// across clones). The SINGLETON stamps the matching id back on so getElementById
// (swQ's singleton path + project.js's swath serialize/restore) resolves them;
// clones keep no id (resolved purely by data-sw within their root).
function swStampSingletonIds(root, $scope) {
  if (swIsInst(root) || !$scope) return;
  $scope.querySelectorAll('[data-sw]').forEach(function(el) {
    if (!el.id) el.id = el.getAttribute('data-sw');
  });
}
...
var swathInstances = {};
// ws-v2 phase 1: which dataset this panel swaths as PRIMARY (its geometry/vars
// drive the run; every OTHER analyzed+gridded dataset overlays as a comparison).
// Default 'model' → bit-identical. Singleton in this module global; clone in its
// swathInstances[id].
var swathTargetDsId = 'model';
var _swSingleton = {
  get swathTargetDsId() { return swathTargetDsId; }, set swathTargetDsId(v) { swathTargetDsId = v; },
...
// Swath runs its OWN worker fan-out (unlike Stats/Cat, which re-read a cached
// analysis). swathTargetDs(root) is the panel's PRIMARY; swathCtx(root) resolves
// its analysis header/geometry/xyz; swathRunInputs(ds) builds the worker message
...
function swathCtx(root) {
  ...
  if (isModel) {
    ...
    return {
      ...
      header: d.header || currentHeader || [],
      colTypes: d.colTypes || currentColTypes || [],
      geometry: d.geometry || (lastCompleteData && lastCompleteData.geometry) || null,
      xyz: currentXYZ, dxyz: currentDXYZ
    };
  }
  var c = ds.complete || {}, pf = ds.preflight || {};
  return {
    ds: ds, id: ds.id, isModel: false,
    header: c.header || pf.header || [],
    colTypes: c.colTypes || pf.autoTypes || [],
    geometry: c.geometry || null,
    xyz: pf.xyz || { x: -1, y: -1, z: -1 }, dxyz: { dx: -1, dy: -1, dz: -1 }
  };
}
// Worker message inputs (+ resolved weight) for a dataset's swath run. Branches
// model (resolved globals) vs comparison (preflight) — the model can't go through
// the raw-preflight path because it uses its RESOLVED xyz/types/calcols.
function swathRunInputs(ds) {
  if (ds.id === 'model') {
    return {
      file: currentFile,
  ...
  return {
    ...
    weight: resolveSwathCmpWeight(ds)
  };
}
// The "Dataset" picker host at the top of the sidebar — shown at 2+ swath-able
// datasets. onchange re-wired each render (mirrors statsCat).
...
// Switch the panel's primary dataset and rebuild its sidebar for that dataset.
function setSwathTarget(id, root) {
  var S = swStateForRoot(root);
  if (id === S.swathTargetDsId) return;
  S.swathTargetDsId = id;
  S.lastSwathData = null;
  renderSwathConfig(undefined, root);
  renderSwathOutput(undefined, root);
...
function swSerializeConfig(root) {
  ...
  var primHeader = swathCtx(root).header || currentHeader || [];   // ws-v2 phase 1: primary var names vs the TARGET header
  var checkedVars = [];
  if ($vl) $vl.querySelectorAll('input[type="checkbox"]:not([data-aux]):checked').forEach(function(cb) {
    var name = primHeader[parseInt(cb.value)];
  ...
  return {
    targetDsId: swStateForRoot(root).swathTargetDsId || 'model',   // ws-v2 phase 1
...
function swApplyConfig(root, cfg) {
  if (!cfg) return;
  // ws-v2 phase 1: switch to the saved target FIRST so the sidebar (directions,
  // primary vars, geometry) is rebuilt for it before we apply the saved controls.
  if (cfg.targetDsId && cfg.targetDsId !== (swStateForRoot(root).swathTargetDsId || 'model')) {
    swStateForRoot(root).swathTargetDsId = cfg.targetDsId;
    renderSwathConfig(undefined, root);
  }
  var $dirMode = swQ('swathDirMode', root);
...
function swathCmpDatasets(root) {
  var tid = swathTargetDs(root).id;
  var out = [];
...
function swathShownCmpDatasets(root) {
  return swathCmpDatasets(root).filter(function(d) { return !panelState.swath.dsHidden.has(d.id); });
}
...
function showSwathColorPicker(colName, colIdx, anchorEl, presetColor, root) {
  ...
  var sidebar = swQ('swathSidebar', root);
...
function hideSwathColorPicker(root) {
  var $picker = swQ('swathColorPicker', root);
...
function applySwathColor(colName, color, root) {
  ...
  swQA('swathVarList', '.swath-color-swatch', root).forEach(function(sw) {
...
function getSwathUnit(colIdx, header, dsId) {
  header = header || currentHeader; dsId = dsId || 'model';
  var idx = header[colIdx] ? catPropUnit(dsId, header[colIdx]) : 0;
...
function getAuxSwathVarColor(baseName, fallbackIdx, dsId, targetId) {
  dsId = dsId || 'aux';
  targetId = targetId || 'model';
  var rec = catVarPeek(dsId, baseName);
  if (rec && rec.color) return rec.color;
  var p = catMemberIn(dsId, baseName, targetId);   // the primary (target) column this comparison shares a property with
  if (p) {
    for (var i = 0; i < swathNumCols.length; i++) {
      if (swathNumCols[i].name === p) return getSwathVarColor(p, i, targetId);
    }
  }
...
function getSwathDisplay(root) {
  var b = swQ('swathShowBands', root), c = swQ('swathShowCounts', root), t = swQ('swathShowTable', root);
  return {
    showBands: b ? b.checked : true,
    showCounts: c ? c.checked : true,
    showTable: t ? t.checked : true,
    yScale: (swQ('swathYScale', root) || {}).value || 'linear',
    layout: (swQ('swathLayout', root) || {}).value || 'overlay'
  };
}
...
function swathDirView(sd, key) {
  ...
  return {
    ...
    varCols: sd.varCols,
    header: sd.header || null,            // ws-v2 phase 1: the primary (target) header
    targetDsId: sd.targetDsId || 'model', // and which dataset is primary (cmp pairing anchor)
...
function renderSwathOutput(stat, root) {
  ...
  renderSwathCharts(swathDirView(sd, sd.activeKey), stat, $content.querySelector('#swathDirPanel'), root);
}

function renderSwathConfig(data, root) {
  if (data) _swathData = data;   // s-4b: cache the MODEL analysis (clone builds + the model ctx)
  const $sidebar = swQ('swathSidebar', root);
  const $content = swQ('swathContent', root);
  if (!$sidebar) return;
  // ws-v2 phase 1: the panel's PRIMARY dataset (model default → resolves to the
  // current* globals, bit-identical). A picker section lets the user switch.
  var ctx = swathCtx(root);
  var dsTag = ctx.id;
  const header = ctx.header, colTypes = ctx.colTypes, geometry = ctx.geometry;
  const xyz = ctx.xyz, dxyz = ctx.dxyz;
  const pickerHtml = '<div class="swath-sidebar-section" data-sw="swathDatasetWrap"></div>';
  const hasXYZ = xyz.x >= 0 && xyz.y >= 0 && xyz.z >= 0;
  if (!hasXYZ || !geometry || !geometry.x) {
    $sidebar.innerHTML = pickerHtml;
    swStampSingletonIds(root, $sidebar);
    renderSwathDatasetPicker(root);
    $content.innerHTML = '<div class="swath-hint">Assign X/Y/Z columns for ' + esc(dsLabel(dsTag)) + ' to enable Swath plots.</div>';
    return;
  }
  swathNumCols = header.map((h, i) => ({ name: h, idx: i, type: colTypes[i] }))
    .filter(c => c.type === 'numeric' && c.idx !== xyz.x && c.idx !== xyz.y && c.idx !== xyz.z
      && c.idx !== dxyz.dx && c.idx !== dxyz.dy && c.idx !== dxyz.dz);
  ...
  function dirRow(key, label, bs, checked, title) {
    return '<div class="swath-dir-row">' +
      ...
      '<input type="number" class="swath-input swath-dir-bin" data-sw="swathBin_' + key + '" value="' + bs + '" min="0.001" step="any" title="Bin width (m)">' +
    '</div>';
  }
  const varItems = swathNumCols.map(function(c, vi) {
    var defUnit = catPropUnit(dsTag, c.name);
    var unitOpts = GRADE_UNITS.map(function(u, ui) {
      return '<option value="' + ui + '"' + (ui === defUnit ? ' selected' : '') + '>' + esc(u.label) + '</option>';
    }).join('');
    var varColor = getSwathVarColor(c.name, vi, dsTag);
    var emptyTag = colIsEmpty(dsTag, c.idx) ? '<span class="empty-tag" title="' + EMPTY_COL_TITLE + '">∅</span>' : '';
  ...
  $sidebar.innerHTML =
    ...
    '<div class="swath-sidebar-section" data-sb="dir">' +
      ...
      '<select class="swath-select" data-sw="swathDirMode">' +
      ...
      '<div data-sw="swathOrthoDirs">' +
        dirRow('x', 'X', bsx, true) +
        dirRow('y', 'Y', bsy, false) +
        dirRow('z', 'Z', bsz, false) +
      '</div>' +
      '<div data-sw="swathCustomDirs" style="display:none">' +
        '<div class="swath-angle-row">' +
          '<label title="Dip direction (\u00b0 from N, clockwise)">Dipdir<input type="number" class="swath-input" data-sw="swathDipDir" value="0" min="0" max="360" step="1"></label>' +
          '<label title="Dip (\u00b0 below horizontal)">Dip<input type="number" class="swath-input" data-sw="swathDip" value="0" min="0" max="90" step="1"></label>' +
          '<label title="Rake of U in the plane (\u00b0 from strike, 90 = down-dip)">Rake<input type="number" class="swath-input" data-sw="swathRake" value="90" min="-180" max="180" step="1"></label>' +
        '</div>' +
    ...
    '<div class="swath-sidebar-section" data-sb="stat">' +
      ...
      '<select class="swath-select" data-sw="swathStat">' +
      ...
      '<div style="margin-top:0.3rem">' +
        '<div class="swath-sidebar-title">Weight' + (ctx.isModel ? ' (model)' : '') + '</div>' +
        '<select class="swath-select" data-sw="swathWeight"><option value="">\u2014 none</option>' +
          swathNumCols.map(function(c) { return '<option value="' + esc(c.name) + '"' + (c.name === catRole(dsTag, 'weight') ? ' selected' : '') + '>' + esc(c.name) + '</option>'; }).join('') +
        '</select>' +
      '</div>' +
    '</div>' +
    '<div class="swath-sidebar-section" data-sb="display">' +
      '<div class="swath-sidebar-title">Display</div>' +
      '<label class="swath-display-opt"><input type="checkbox" data-sw="swathShowBands" checked> Show bands</label>' +
      '<label class="swath-display-opt"><input type="checkbox" data-sw="swathShowCounts" checked> Show count bars</label>' +
      '<label class="swath-display-opt"><input type="checkbox" data-sw="swathShowTable" checked> Show table</label>' +
      '<div style="margin-top:0.3rem">' +
        '<div class="swath-sidebar-title">Y Scale</div>' +
        '<select class="swath-select" data-sw="swathYScale"><option value="linear">Linear</option><option value="log">Log</option></select>' +
      '</div>' +
      '<div style="margin-top:0.3rem">' +
        '<div class="swath-sidebar-title">Layout</div>' +
        '<select class="swath-select" data-sw="swathLayout"><option value="overlay">Overlay</option><option value="split">Split</option></select>' +
      '</div>' +
    '</div>' +
    '<div class="swath-sidebar-section" data-sb="filter">' +
      '<div class="swath-sidebar-title">Local Filter</div>' +
      '<input type="text" class="swath-search" data-sw="swathLocalFilter" placeholder="e.g. r.zone == 1" autocomplete="off" spellcheck="false">' +
    '</div>' +
    '<div class="swath-sidebar-section--grow" data-sb="vars">' +
      ...
      '<div data-sw="swathDatasetsSection" style="display:none;margin-bottom:0.4rem">' +
        '<div class="stats-ds-chips" data-sw="swathDatasetChips"></div>' +
      '</div>' +
      '<input type="text" class="swath-search" data-sw="swathVarSearch" placeholder="search\u2026" autocomplete="off" spellcheck="false">' +
      '<div class="swath-var-btns">' +
        '<button data-sw="swathVarAll">All</button>' +
        '<button data-sw="swathVarNone">None</button>' +
      '</div>' +
      '<div class="swath-var-list" data-sw="swathVarList">' + varItems + '<div data-sw="swathAuxVars"></div></div>' +
    '</div>' +
    '<div class="sb-footer">' +
      '<button class="swath-generate" data-sw="swathGenerate">Generate</button>' +
      '<div class="gen-stale-note">↻ config changed — re-run</div>' +
      '<div class="swath-progress" data-sw="swathProgress">' +
        '<div class="swath-progress-bar"><div class="swath-progress-fill" data-sw="swathProgressFill"></div></div>' +
        '<div class="swath-progress-label" data-sw="swathProgressLabel"></div>' +
      '</div>' +
    '</div>' +
    '<div class="swath-color-picker" data-sw="swathColorPicker"></div>';

  // s-4a: the generated controls carry data-sw (duplicate-safe across clones).
  // On the SINGLETON, stamp the matching id so swQ's getElementById path AND the
  // external project.js reads (#swathVarList, getElementById('swathDirMode')\u2026)
  // keep working unchanged; clones stay id-free so the DOM has no duplicate ids.
  swStampSingletonIds(root, $sidebar);

  // ws-v2 phase 1: populate + wire the Dataset picker (shown at 2+ swath-able datasets)
  renderSwathDatasetPicker(root);
  ...
  $dirMode.addEventListener('change', function() {
    swQ('swathOrthoDirs', root).style.display = $dirMode.value === 'ortho' ? '' : 'none';
    swQ('swathCustomDirs', root).style.display = $dirMode.value === 'custom' ? '' : 'none';
  });

  // Variable search filter (fuzzy)
  var $swathVarSearch = swQ('swathVarSearch', root);
  $swathVarSearch.addEventListener('input', function() {
    var q = this.value.toLowerCase();
    swQA('swathVarList', '.swath-var-item', root).forEach(function(item) {
      var name = item.querySelector('span').textContent.toLowerCase();
      item.style.display = fuzzyMatch(q, name) ? '' : 'none';
    });
  });
  wireSearchShortcuts($swathVarSearch, swQ('swathVarAll', root), swQ('swathVarNone', root));

  // All/None buttons — only affect visible (non-filtered) items
  swQ('swathVarAll', root).addEventListener('click', function() {
    swQA('swathVarList', '.swath-var-item', root).forEach(function(item) {
      if (item.style.display !== 'none') item.querySelector('input[type="checkbox"]').checked = true;
    });
  });
  swQ('swathVarNone', root).addEventListener('click', function() {
    swQA('swathVarList', '.swath-var-item', root).forEach(function(item) {
  ...
  var $swathDsChips = swQ('swathDatasetChips', root);
  ...
  // Generate button
  swQ('swathGenerate', root).addEventListener('click', function() { runSwath(root); });

  // Shared support weight (D3): writing here also re-points the Statistics
  // tab's weight select — one role, two views
  swQ('swathWeight', root).addEventListener('change', function() {
    var wctx = swathCtx(root);
    catSetRole(wctx.id, 'weight', this.value || null);
    if (wctx.isModel) {
      var $ssel = document.getElementById('statsWeightSel');
      if ($ssel) $ssel.value = this.value || '';
      markAnalysisStale();
    } else if (typeof markAuxStale === 'function') {
      markAuxStale(wctx.ds, (typeof dsConfigRoot === 'function') ? dsConfigRoot(wctx.ds) : null);
    }
    autoSaveProject();
  });
  ...
  swQ('swathVarList', root).addEventListener('change', function(e) {
    if (e.target.classList.contains('swath-var-unit')) {
      ...
      if (e.target.dataset.auxCol != null) {
        ...
        var uctx = swathCtx(root);
        var un = uctx.header[parseInt(e.target.dataset.col)];
        if (un) catSetUnit(uctx.id, un, uv);
      }
  ...
  swQ('swathVarList', root).addEventListener('click', function(e) {
    ...
    if (swatch) {
      ...
      if (swatch.dataset.colorKey) {
        ...
        var curColor = getAuxSwathVarColor(swatch.dataset.auxName, swathNumCols.length + parseInt(swatch.dataset.auxIdx || 0), swDsId, swathTargetDs(root).id);
        showSwathColorPicker(swatch.dataset.colorKey, null, swatch, curColor, root);
        return;
      }
      var colIdx = parseInt(swatch.dataset.col);
      showSwathColorPicker(swathCtx(root).header[colIdx], colIdx, swatch, undefined, root);
    }
  });

  // Color picker delegation
  var $swathPicker = swQ('swathColorPicker', root);
  $swathPicker.addEventListener('click', function(e) {
    ...
    if (swatch) {
      ...
      applySwathColor(cn, swatch.dataset.color, root);
      hideSwathColorPicker(root);
    }
  });
...
function renderSwathDatasetChips(root) {
  var section = swQ('swathDatasetsSection', root);
  if (!section) return;
  var allCmp = swathCmpDatasets(root);
  if (allCmp.length < 2) { section.style.display = 'none'; return; }
  var primaryId = swathTargetDs(root).id;
  var chips = '<span class="stats-ds-chip stats-ds-chip--model" title="the primary series">' + esc(dsLabel(primaryId)) + '</span>';
  ...
  swQ('swathDatasetChips', root).innerHTML = chips;
...
function renderSwathAuxVars(root) {
  var wrap = swQ('swathAuxVars', root);
  if (!wrap) return;
  ...
  renderSwathDatasetChips(root);
  var cmpDatasets = swathShownCmpDatasets(root);
  if (swathCmpDatasets(root).length === 0) { wrap.innerHTML = ''; return; }
  var primaryId = swathTargetDs(root).id;   // ws-v2 phase 1: pair comparisons to the primary, not always the model
  catEnsureSeeded();
  ...
  cmpDatasets.forEach(function(ds) {
    ...
    for (var k = 0; k < cols.length; k++, ai++) {
      ...
      else checked = !!catMemberIn(ds.id, c.name, primaryId);    // paired-to-primary by default on first appearance
      var unitIdx = catPropUnit(ds.id, c.name);
      var unitOpts = GRADE_UNITS.map(function(u, ui) {
        return '<option value="' + ui + '"' + (ui === unitIdx ? ' selected' : '') + '>' + esc(u.label) + '</option>';
      }).join('');
      var color = getAuxSwathVarColor(c.name, swathNumCols.length + ai, ds.id, primaryId);
...
function getSwathDirections(root) {
  var mode = (swQ('swathDirMode', root) || {}).value || 'ortho';
  var $sb = swQ('swathSidebar', root);
  var dirs = [];
  function pushDir(key, axis, vec) {
    var cb = $sb && $sb.querySelector('.swath-dir-on[data-dir="' + key + '"]');
    if (!cb || !cb.checked) return;
    var bw = parseFloat((swQ('swathBin_' + key, root) || {}).value);
  ...
  if (mode === 'ortho') {
    ...
    var dipDir = parseFloat((swQ('swathDipDir', root) || {}).value) || 0;
    var dip = parseFloat((swQ('swathDip', root) || {}).value) || 0;
    var rake = parseFloat((swQ('swathRake', root) || {}).value);
...
function swathConfigError(msg, root) {
  var $progress = swQ('swathProgress', root);
  var $label = swQ('swathProgressLabel', root);
...
function runSwath(root) {
  ...
  var dcfg = getSwathDirections(root);
  if (dcfg.dirs.length === 0) {
    swathConfigError('Check at least one direction', root);
    return;
  }
  for (var di = 0; di < dcfg.dirs.length; di++) {
    var dbw = dcfg.dirs[di].binWidth;
    if (!dbw || dbw <= 0 || isNaN(dbw)) {
      swathConfigError('Invalid bin width for ' + dcfg.dirs[di].key.toUpperCase(), root);
      return;
    }
  }

  // ws-v2 phase 1: the PRIMARY dataset this panel swaths (model default). Its
  // worker inputs come from swathRunInputs (resolved globals for the model;
  // preflight for a comparison). The primary and each comparison live in
  // separate header/index spaces.
  var ctx = swathCtx(root);
  var targetDs = swathTargetDs(root);
  var primIn = swathRunInputs(targetDs);

  // Gather selected PRIMARY variable column indices (into the target's header).
  var varCols = [];
  swQA('swathVarList', 'input[type="checkbox"]:not([data-aux]):checked', root).forEach(function(cb) {
    varCols.push(parseInt(cb.value));
  });
  // A10 4c-iii-b: build a run per comparison dataset (every shown dataset EXCEPT
  // the target) that has selected vars — one swath worker each, resolving its own
  // weight (declustering-stale soft-fails into a per-dataset error, no worker).
  var cmpRuns = [];
  swathShownCmpDatasets(root).forEach(function(ds) {
    if (!swathCmpReady(ds)) return;
    var sel = [];
    swQA('swathVarList', 'input[type="checkbox"][data-aux][data-ds="' + ds.id + '"]:checked', root).forEach(function(cb) {
      sel.push(parseInt(cb.value));
    });
    if (sel.length === 0) return;
    cmpRuns.push({ ds: ds, varCols: sel, inputs: swathRunInputs(ds) });
  });
  if (varCols.length === 0 && cmpRuns.length === 0) return;
  // If the primary itself is weight-blocked (e.g. a comparison target with stale
  // declustered weights), surface it rather than launching a bad run.
  if (varCols.length > 0 && primIn.weight.blockedError) {
    swathConfigError(primIn.weight.blockedError, root);
    return;
  }
  ...
  // The progress bar follows the primary worker when present, else the first
  // comparison worker (so a comparison-only run still animates).
  var barDriverId = varCols.length > 0 ? targetDs.id : (cmpRuns[0] ? cmpRuns[0].ds.id : null);

  var $progress = swQ('swathProgress', root);
  var $fill = swQ('swathProgressFill', root);
  var $label = swQ('swathProgressLabel', root);
  var $content = swQ('swathContent', root);
  $progress.classList.add('active');
  $fill.style.width = '0%';
  ...
  var $btn = swQ('swathGenerate', root);
  ...
  function finalize() {
    ...
    for (var ci = 0; ci < datasets.length; ci++) {
      if (datasets[ci].id === targetDs.id) continue;   // the primary isn't a comparison of itself
    ...
    S.lastSwathData = {
      ...
      cmp: cmp,
      targetDsId: targetDs.id, header: ctx.header,   // ws-v2 phase 1: primary var names + cmp pairing anchor
  ...
  if (varCols.length > 0) {
    ...
    S.swathWorker.postMessage({
      mode: 'swath',
      file: primIn.file,
      zipEntry: primIn.zipEntry,
      globalFilter: primIn.globalFilter,
      localFilter: localFilter || null,
      calcolCode: primIn.calcolCode,
      calcolMeta: primIn.calcolMeta,
      resolvedTypes: primIn.resolvedTypes,
      xyzCols: primIn.xyzCols,
      dxyzCols: primIn.dxyzCols,
      directions: workerDirs,
      varCols: varCols,
      rowVarOverride: primIn.rowVarOverride,
      weightColName: primIn.weight.weightColName,
      weightArray: primIn.weight.weightArray,
      dmEndianness: primIn.dmEndianness,
      dmFormat: primIn.dmFormat
    });
  ...
  cmpRuns.forEach(function(run) {
    ...
    out.cmp[ds.id] = slot;
    if (inp.weight.blockedError) { slot.error = inp.weight.blockedError; return; }  // no worker
    ...
    w.postMessage({
      mode: 'swath',
      file: inp.file,
      zipEntry: inp.zipEntry,
      globalFilter: inp.globalFilter,
      localFilter: null,
      calcolCode: inp.calcolCode,
      calcolMeta: inp.calcolMeta,
      resolvedTypes: inp.resolvedTypes,
      xyzCols: inp.xyzCols,
      dxyzCols: inp.dxyzCols,
      directions: workerDirs,
      varCols: run.varCols,
      rowVarOverride: inp.rowVarOverride,
      weightColName: inp.weight.weightColName,
      weightArray: inp.weight.weightArray,
      dmEndianness: inp.dmEndianness,
      dmFormat: inp.dmFormat
    });
...
function renderSwathCharts(swathData, stat, $target, root) {
  var $content = $target || swQ('swathContent', root);
  ...
  // ws-v2 phase 1: primary var names/units/colours resolve through the TARGET
  // header + dataset (stored on the result), not the model globals.
  var primHeader = swathData.header || currentHeader || [];
  var targetId = swathData.targetDsId || 'model';
  var noData = [];
  var varEntries = [];
  for (var vi = 0; vi < varCols.length; vi++) {
    ...
    var name = primHeader[colIdx] || 'Variable';
    ...
    varEntries.push({
      ...
      isAux: false,
      name: name,
      color: getSwathVarColor(name, vi, targetId),
      bins: bins,
      centerLine: lines.centerLine,
      upperLine: lines.upperLine,
      lowerLine: lines.lowerLine,
      unit: getSwathUnit(colIdx, primHeader, targetId),
      scaleKey: name
    });
  }
  ...
  cmp.forEach(function(c) {
    ...
    for (var ai = 0; ai < cVarCols.length; ai++) {
      ...
      var aPair = catMemberIn(c.dsId, baseName, targetId);
      ...
      varEntries.push({
        ...
        color: getAuxSwathVarColor(baseName, varEntries.length, c.dsId, targetId),
        bins: aBins,
  ...
  var display = getSwathDisplay(root);
  var chartSvg = display.layout === 'split'
    ? renderSwathSplitSvg(varEntries, swathData, stat, display, root)
    : renderSwathOverlaySvg(varEntries, swathData, stat, display, root);
  var html = '';
  cmp.forEach(function(c) {
...
function renderSwathOverlaySvg(varEntries, swathData, stat, display, root) {
  ...
  var W = chartHostWidth(swQ('swathContent', root), 720 + extraAxes * 55, 560 + extraAxes * 55, 34);
...
function renderSwathSplitSvg(varEntries, swathData, stat, display, root) {
  var bw = swathData.binWidth;
  ...
  // Dimensions
  var W = chartHostWidth(swQ('swathContent', root), 720, 560, 34);
...
function wireSwathTableEvents($scope) {
  var $content = $scope || swQ('swathContent');
  if (!$content) return;
  ...
  $content.querySelectorAll('[data-swath-copy]').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      ...
      var table = $content.querySelector('#swathTableBody' + idx + ' .swath-table');
      if (!table) return;
      var tsv = [];
  ...
  $content.querySelectorAll('[data-swath-collapse]').forEach(function(hdr) {
    hdr.addEventListener('click', function() {
      ...
      var body = $content.querySelector('#swathTableBody' + idx);
