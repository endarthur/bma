...
function renderSwathCharts(swathData, stat) {
  ...
  var auxVarCols = swathData.auxVarCols || [];
  var hasData = varCols.some(function(vi) { return swathData.vars[vi] && swathData.vars[vi].length > 0; }) ||
    auxVarCols.some(function(vi) { return swathData.auxVars && swathData.auxVars[vi] && swathData.auxVars[vi].length > 0; });
  if (!hasData) {
  ...
  if (swathData.auxVars) {
    ...
    for (var ai = 0; ai < auxVarCols.length; ai++) {
      ...
      varEntries.push({
        ...
        color: getAuxSwathVarColor(baseName, varEntries.length),
        bins: aBins,
