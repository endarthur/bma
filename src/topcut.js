...
function auxTopcutHistSvg() {
  ...
  var x0 = v[0], x1 = v[m - 1];
  if (x1 <= x0) x1 = x0 + 1;
  var ax = tcAxis(x0, x1, false);
  var nb = 50, bins = new Array(nb).fill(0);
  for (var i = 0; i < m; i++) {
    var b = Math.min(nb - 1, Math.floor((v[i] - x0) / (x1 - x0) * nb));
    bins[b]++;
  }
  ...
  return tcSvgOpen(ax) + tcXTicks(ax, x0, x1, false) + bars + tcCapMarker(ax) + tcFrame('Histogram', 'count') + '</svg>';
}
...
function auxTopcutMetalSvg() {
  ...
  return tcSvgOpen(ax) + grid + tcXTicks(ax, x0, x1, false) +
    '<path d="' + path + '" fill="none" stroke="var(--amber)" stroke-width="1.4"/>' +
    tcCapMarker(ax) + tcFrame('Metal removed by cap', '') + '</svg>';
}
