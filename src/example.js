...
'',
'## Opening tabs as you go',
'',
'A fresh workspace shows only Import Model, Statistics and Categories. The',
'steps below say "open the GT tab", "open the Swath tab", "open the Aux tab"',
'and so on — open any panel that is not showing from View > Panels (open ones',
'are checked), from the data tree, or with the + on the tab strip. (The packed',
'example project restores all of them automatically, so the shortcut above',
'skips this entirely.)',
'',
'## 1. Load the model',
'',
'Drop bma-example-model.csv on the landing page. Import Model detects the',
'delimiter, types, and X/Y/Z automatically — just hit Analyze (bottom right).',
...
'Try Weight: select DENSITY and re-run Analyze — statistics become',
...
'see total weight. Set Weight back to none and Analyze again.',
'',
...
'',
'Click Simulate to preview, then Analyze. RATIO appears in Statistics and',
...
'',
'Add a comparison dataset — Data > Add point dataset (or open the Aux tab',
'from View > Panels) — and drop bma-example-samples.csv. EAST/NORTH/ELEV are',
