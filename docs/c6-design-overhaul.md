...
| **C6-2** | **Menubar** (desktop ≥701px): `MenuBar` from the already-vendored @gcu/menu across the top. File (open / **Open recent ▸ — the recents store gains a proper home beyond the landing page** / save/pack/load/clear), View (Panels with checkmarks — the canonical reopen-closed-tabs home — reset layout, tree, theme, UI scale), Data (analyze/filter/calc/aux), Help (tutorial, manual, F1 overlay, about+version). Absorbs the toolbar ⋮ AND the SAVE/PACK/LOAD/CLEAR button row (D5 amendment — toolbar slims to filename + stats + filter). Kebab stays <701px. Layout presets land here later (View → Layouts) | M |
