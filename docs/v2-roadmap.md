...
| A9 | **Data-fidelity pass: zero row loss + parse-failure surfacing + silent-behavior audit fixes** (Arthur 2026-06-11, prompted by A8's detection-starvation find: "are there any other surprises lurking?") | M | A8 | **Audit done 2026-06-11; findings F1–F8 + 4-phase plan: [`docs/a9-data-fidelity.md`](a9-data-fidelity.md).** Headlines: model analyze still loses detection-warmup rows (aux already immune — Phase 1 copies its full-preflight-types pattern, main-thread only); parse failures invisible (conflated with sentinel nulls; preflight's 0.8 threshold can silently null 20% of a mixed column) → per-column parseFails counter + MIXED badges; filter/calcol per-row exceptions silently exclude rows, uncounted; GT turns invalid weight/density into 1 while analyze excludes+counts; GT groupBy collapses category 0 into ''; geometry accepts sentinel coords (-999 = real grid position); group caps (500/200) overflow silently; ragged/quoted lines misparse with no counter |
